...
import com.lyapi.lyapisdk.exception.ErrorCode;
import com.lyapi.lyapisdk.exception.ErrorResponse;
import com.lyapi.lyapisdk.utils.SignUtils;
import com.lyapi.lyapisdk.model.request.BaseRequest;
...
public abstract class BaseService implements ApiService {
    private LyApiClient lyApiClient;
    /**
     * 网关HOST
     */
     ...
     */
    public void checkConfig(LyApiClient lyApiClient) throws ApiException {
        if (lyApiClient == null && this.getLyApiClient() == null) {
            throw new ApiException(ErrorCode.NO_AUTH_ERROR, "请先配置密钥AccessKey/SecretKey");
        }
        if (lyApiClient != null && !StringUtils.isAnyBlank(lyApiClient.getAccessKey(), lyApiClient.getSecretKey())) {
            this.setLyApiClient(lyApiClient);
        }
    }
    ...
    private <O, T extends ResultResponse> HttpRequest getHttpRequestByRequestMethod(BaseRequest<O, T> request) throws ApiException {
        ...
        return httpRequest.addHeaders(getHeaders(JSONUtil.toJsonStr(request), lyApiClient)).body(JSONUtil.toJsonStr(request.getRequestParams()));
    }
     ...
     */
    private Map<String, String> getHeaders(String body, LyApiClient lyApiClient) {
        Map<String, String> hashMap = new HashMap<>(4);
        hashMap.put("accessKey", lyApiClient.getAccessKey());
        String encodedBody = SecureUtil.md5(body);
        hashMap.put("body", encodedBody);
        hashMap.put("timestamp", String.valueOf(System.currentTimeMillis() / 1000));
        hashMap.put("sign", SignUtils.getSign(encodedBody, lyApiClient.getSecretKey()));
        return hashMap;
    }
    ...
    @Override
    public <O, T extends ResultResponse> T request(LyApiClient lyApiClient, BaseRequest<O, T> request) throws ApiException {
        checkConfig(lyApiClient);
